...
### Backend
- **Entitlement service (new):** `app/services/report_entitlement_service.py` — `resolve_entitlement(db, user, full_name, birth_day)` returns `(tier, unlocked_sections, matched_order_id, pdf_source)`. A logged-in user is `paid` (ALL_SECTIONS) when EITHER **premium** (`is_premium_user` = active UserPackage subscription OR remaining `number_download` quota → unlocks any lookup, `pdf_source='quota'`) OR they own a paid **report/combo** order whose `meta` identity (accent/case-insensitive name + birth_day digits) matches (`pdf_source='order'`). Otherwise `free` (FREE_SECTIONS = số chủ đạo + power chart + chu kỳ cá nhân, `pdf_source='free'`). `find_order_report_download_id` maps an order → its fulfilled UserReport PDF id.
...
- **PDF flow:** the download path follows `pdf_source` — `free` → `/api/so-hoc-free` (reduced); `quota` (premium subscriber) → `/api/so-hoc` (full, quota-decrementing); `order` (per-report purchase) → `downloadReportBlob` (`/api/my/reports/{id}/download`). `BoxExportPDF` copy is tier-aware; fixed typo "thần số sọc" → "thần số học".
