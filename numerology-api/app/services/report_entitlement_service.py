...
async def resolve_entitlement(
    ...
    full_name: str,
    birth_day: str,
) -> tuple[str, frozenset[str], Optional[int]]:
    """Return (tier, unlocked_sections, matched_order_id).

    tier: "free" | "paid". Anonymous always free. A logged-in user is "paid" only
    when they own a paid report/combo order whose identity matches this report.
    """
    if user is None:
        return "free", FREE_SECTIONS, None
    ...
    for order in result.scalars().all():
        ...
        if any(
            ...
            return "paid", ALL_SECTIONS, order.id
